...
export function getTemplateById(id) {
  if (!id) return null;
  return PREBUILT_TEMPLATES.find(t => t.id === id || t.id.toLowerCase() === id.toLowerCase()) || null;
}

export function getComponentVariant(sectionKey, variantId) {
  const section = COMPONENT_LIBRARY[sectionKey];
  if (!section) return null;
  return section.variants.find(v => v.id === variantId) || null;
}
